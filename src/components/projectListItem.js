...
export default function ProjectListItem({ project }) {
    return (
        <li key={project.slug} className="relative overflow-hidden">
            <Link href={`/projects/${project.slug}`} className="group before:absolute before:top-0 before:left-0 before:w-full before:h-full before:bg-pink before:translate-y-20 before:transition before:duration-300 hover:before:translate-y-0 ">
